...
def _get_best_pr_for_exercise(db: Session, user_id: str, exercise: str):
    return db.query(PR).filter(
        PR.user_id == user_id, PR.exercise == exercise
...
def _get_best_pr_across_names(db: Session, user_id: str, names: List[str]):
    if not names:
        return None
    return db.query(PR).filter(
        PR.user_id == user_id, PR.exercise.in_(names)
    ).order_by(PR.estimated_1rm.desc()).first()
...
def create_workout_plan(plan: WorkoutPlanCreate, db: Session = Depends(get_db)):
    ...
    for exercise in plan.exercises:
        db.add(Workout(user_id=plan.user_id, workout_letter=plan.workout_letter,
...
def complete_workout(completion: WorkoutCompletionUpdate, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(record)
    return {"workout_letter": record.workout_letter, "completion_count": record.completion_count,
...
def create_dashboard_member(member: DashboardMemberCreate, db: Session = Depends(get_db)):
    ...
    if existing:
        return DashboardMemberResponse(user_id=existing.user_id, username=existing.username, full_name=existing.full_name,
            unique_code=existing.unique_code, dashboard_url=f"https://dashboard-production-79f2.up.railway.app/{existing.unique_code}")
    ...
    return DashboardMemberResponse(user_id=new_member.user_id, username=new_member.username, full_name=new_member.full_name,
        unique_code=new_member.unique_code, dashboard_url=f"https://dashboard-production-79f2.up.railway.app/{new_member.unique_code}")
...
def list_all_members(db: Session = Depends(get_db)):
    ...
    return [{"user_id": m.user_id, "username": m.username, "full_name": m.full_name,
             "unique_code": m.unique_code, "dashboard_url": f"https://dashboard-production-79f2.up.railway.app/{m.unique_code}"} for m in members]
...
def dashboard_log_exercise(unique_code: str, body: dict, db: Session = Depends(get_db)):
    ...
    if best:
        ...
        is_pr = True
    new_pr = PR(user_id=member.user_id, username=member.username, exercise=store_as, weight=weight, reps=reps,
        estimated_1rm=estimated_1rm, message_id=f"dashboard-{datetime.utcnow().isoformat()}", channel_id="dashboard", timestamp=datetime.utcnow())
    db.add(new_pr)
    now = datetime.utcnow()
...
def dashboard_log_workout(unique_code: str, workout_data: dict, db: Session = Depends(get_db)):
    ...
    for ex in exercises:
        if ex.get("weight", 0) > 0 or ex.get("reps", 0) > 0:
            ...
            db.add(PR(user_id=member.user_id, username=member.username, exercise=ex["name"],
                weight=ex.get("weight", 0), reps=ex.get("reps", 0), estimated_1rm=estimated_1rm,
                message_id=f"dashboard-{datetime.utcnow().isoformat()}", channel_id="dashboard", timestamp=datetime.utcnow()))
